...
namespace SteerLib
{
	...
	bool AStarPlanner::computePath(std::vector<Util::Point>& agent_path, Util::Point start, Util::Point goal, SteerLib::SpatialDataBaseInterface * _gSpatialDatabase, bool append_to_path)
	{
		...
		gridcell[x_s][z_s].f = gridcell[x_s][z_s].g + calcH_by_Euclidian(x_s, z_s, x_g, z_g);;
		...
		while (gridcell[x_g][z_g].g - smallest_F > -err)
		{
			...
			if (x_range_max == x_n + 1)
			{
				...
				if (canBeTraversed(index_neighbour))
				{
					if (gridcell[x_n + 1][z_n].g - gridcell[x_n][z_n].g - cellsizeX > 0)
					{
						...
						gridcell[x_n + 1][z_n].f = gridcell[x_n + 1][z_n].g + calcH_by_Euclidian(x_n + 1, z_n, x_g, z_g);;
			...
			if (z_range_max == z_n + 1)
			{
				...
				if (canBeTraversed(index_neighbour))
				{
					if (gridcell[x_n][z_n + 1].g - gridcell[x_n][z_n].g - cellsizeZ > 0)
					{
						...
						gridcell[x_n][z_n + 1].f = gridcell[x_n][z_n + 1].g + calcH_by_Euclidian(x_n, z_n + 1, x_g, z_g);
						//delete from the list if already presented
			...
			if (x_range_max == x_n + 1 && z_range_min == z_n - 1)
			{
				...
				if (canBeTraversed(index_neighbour))
				{
					if (gridcell[x_n + 1][z_n - 1].g - gridcell[x_n][z_n].g - std::sqrt(cellsizeX*cellsizeX + cellsizeZ*cellsizeZ) > 0)
					{
						...
						gridcell[x_n + 1][z_n - 1].f = gridcell[x_n + 1][z_n - 1].g + calcH_by_Euclidian(x_n + 1, z_n - 1, x_g, z_g);
						//delete from the list if already presented
